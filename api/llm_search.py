...
async def llm_search_endpoint(payload: LLMSearchIn, current_user: dict = Depends(get_current_user)):
    ...
    if payload.subject_id:
        ...
        try:
            # reuse conversation append helper (pdf_filename not relevant)
            await append_conversation_to_subject(payload.subject_id, payload.query, result, pdf_filename=None, added_by=current_user.get("_id"))
        except Exception as e:
